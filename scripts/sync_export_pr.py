...
def _render_pr_body(
    *,
    ...
    body = f"{body.rstrip()}\n{footer}\n"
    if len(_state_marker_lines(body)) != 1:
...
def _applied_marker_from_pr_body(body: str) -> str:
    ...
    markers = _state_marker_lines(body)
    if len(markers) > 1:
...
def _state_marker_lines(body: str) -> list[str]:
    """Return exact Copybarista state marker lines."""
    return [line for line in body.splitlines() if line.startswith(PR_MARKER_PREFIX)]
...
def _maybe_resolve_source_marker(
    *, source_dir: Path, marker: str, marker_source: str
    ...
    if marker.startswith("sha:"):
        return _maybe_resolve_source_rev(
            source_dir=source_dir,
            rev=marker.removeprefix("sha:"),
        )
    if marker.startswith("sha256:"):
...
def _maybe_resolve_source_rev(*, source_dir: Path, rev: str) -> str:
    """Resolve a source revision if it remains reachable."""
    result = _run(
    ...
    return rev if result.returncode == 0 else ""
...
def _body_before_footer(body: str) -> str:
    ...
    lines = body.splitlines()
    marker_indexes = [
        idx for idx, line in enumerate(lines) if line.startswith(PR_MARKER_PREFIX)
    ]
    if not marker_indexes:
        raise PrReplayError("PR body is missing the Copybarista export footer.")
    marker_idx = marker_indexes[-1]
    separator_idx = marker_idx - 1
    while separator_idx >= 0 and lines[separator_idx] != "----":
        separator_idx -= 1
    if separator_idx < 0:
        raise PrReplayError("PR body is missing the Copybarista export footer.")
