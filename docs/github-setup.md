...
In the older example workflows, `COPYBARISTA_SOURCE_PROJECT_PATH` is the
directory that contains `copy.barista.toml` and the `source_ref` passed to
Copybarista. The scaffolded package workflow generated by `init-sync` uses
`COPYBARISTA_SOURCE_ROOT`, matching the `source_root` field in
...
repository policy differs. Run `init-sync --force` only when intentionally
regenerating existing sync files. The public import workflow deliberately separates
validation from PR creation: import verification runs without `GH_TOKEN`, then a
second `--open-pr-only` step receives the token only after trusted helper code is
captured from the source checkout.
...
```yaml
concurrency:
  group: copybarista-export-${{ inputs.project || vars.COPYBARISTA_SOURCE_ROOT }}
  cancel-in-progress: false
```
